...
export default function RawMaterialsImportPage() {
  ...
  useEffect(() => {
    ...
    } else if (user?.role !== "warehouse_raw") {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.role === "warehouse_raw") {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Nhập kho thành công", description: result.message });

      setPurchaseRequests(purchaseRequests.filter(p => p !== maPhieuMuaNVL));
      setMaPhieuMuaNVL("");
      setDetails([]);
    } catch (err: any) {
      setError(err.message);
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-4xl mx-auto">
            <div className="mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-1">Chọn phiếu mua hàng để nhập kho</p>
            </div>
              ...
              <CardContent className="space-y-6">
                {loadingRequests ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-semibold mb-2">Không có phiếu mua hàng nào</h3>
                  ...
                  <>
                    <div>
                      <label className="block text-sm font-medium mb-2">Chọn phiếu mua hàng</label>
                      <Select value={maPhieuMuaNVL} onValueChange={setMaPhieuMuaNVL}>
                        ...
                        <SelectContent>
                          {purchaseRequests.map(pr => (
                            <SelectItem key={pr} value={pr}>Phiếu mua {pr}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    {maPhieuMuaNVL && (
                      <>
                        {loading ? (
                          ...
                          <div className="text-center py-12 text-muted-foreground">Không có nguyên vật liệu nào</div>
                        ) : (
                          <>
                            {Object.entries(groupedByNCC).map(([ncc, items]) => (
                              <div key={ncc}>
                                <h3 className="font-semibold text-lg mb-3 pb-2 border-b">Nhà Cung Cấp: {ncc}</h3>
                                    ...
                                    <tbody>
                                      {items.map((item, idx) => (
                                        <tr key={idx} className="border-b hover:bg-muted/30">
                                          ...
                                          <td className="py-3 px-4 text-right">{item.soLuongYC} {item.donVi}</td>
                                          <td className="py-3 px-4 text-right">{item.donGia.toLocaleString("vi-VN")} đ</td>
                                          <td className="py-3 px-4 text-right font-medium">{(item.soLuongYC * item.donGia).toLocaleString("vi-VN")} đ</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
                            ...
                            <div className="pt-4 border-t flex justify-between items-center">
                              ...
                              <span className="text-2xl font-bold text-primary">{tongGiaTri.toLocaleString("vi-VN")} đ</span>
                            </div>
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t">
                              <Button variant="outline" onClick={() => { setMaPhieuMuaNVL(""); setDetails([]); }} disabled={isSaving}>
                                Hủy
                              </Button>
